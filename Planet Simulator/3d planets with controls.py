...
namelist = ['FPS','Number of Planets','Radius of Planets','Initial Speed','Trail Length','Friction',
            'Gravity','Border Bouncing','Absorbing Collisions','Billiard Collisions'] #set labels
labelsurfs = []
labelrects = []
offset = 20
...
def planetcreate (n):
    ...
    while (count <= n):
        info = [(random.randint(50,255),random.randint(50,255),random.randint(50,255)),
                random.randint(border,screenw-border),random.randint(border,screenh-border),random.randint(frontborder,screend-border),
                random.randint(settings[2],settings[3]),
                0,0,0,
                random.randint(-settings[4],settings[4]),random.randint(-settings[4],settings[4]),random.randint(-settings[4],settings[4])]
...
def planetdraw(planets):
    ...
    for x in planets:
        for n in range(1,len( planets)):
            planet = planets[n]
            prevplanet = planets[n-1]
            if planet[3] > prevplanet[3]:
                changeplanet = planets[n-1]
                planets[n-1] = planets[n]
                planets[n] = changeplanet
    for planet in planets:
        apparentradius = int((float(screend*planet[4])/(4*math.sqrt(planet[3]*planet[3])+1))+3)
        if(planet[3] >=frontborder):
            pygame.draw.circle(DISPLAYSURF,planet[0],(planet[1],planet[2]),apparentradius,0)
...
def gravitycalc(planets):
    ...
    for planet in planets:
        for fplanet in planets:
            if planet != fplanet:
                ...
                DZ = (planet[3]-fplanet[3])
                DIST = math.sqrt(DX*DX + DY*DY + DZ*DZ)
                M = planet[4]**3
                Mf = fplanet[4]**3
                G = 6.67*(10**(-11))
                if DIST > (planet[4]+fplanet[4]) and settings[17]:
                    planet[5] -= (settings[17]*Mf*DX)/(DIST*DIST*DIST)
                    planet[6] -= (settings[17]*Mf*DY)/(DIST*DIST*DIST)
                    planet[7] -= (settings[17]*Mf*DZ)/(DIST*DIST*DIST)
                    
                elif  (DIST <= (planet[4]+fplanet[4])):
                    if(settings[12]):
                        planetgone = planets.pop(planets.index(fplanet))
                        planet[4] = int(round(((M+Mf)**(1.0/3.0))))
                        planet[0] = tuple(map(lambda x, y: (M*x + Mf*y)/(M+Mf), planet[0], planetgone[0]))
                        planet[1] = (M*planet[1] + Mf*planetgone[1])/(M + Mf)
                        planet[2] = (M*planet[2] + Mf*planetgone[2])/(M + Mf)
                        planet[3] = (M*planet[3] + Mf*planetgone[3])/(M + Mf)
                        planet[8] = (M*planet[8] + Mf*planetgone[8])/(M + Mf)
                        planet[9] = (M*planet[9] + Mf*planetgone[9])/(M + Mf)
                        planet[10] = (M*planet[10] + Mf*planetgone[10])/(M + Mf) 
                        
                    if(settings[18]):
                        if(DX!=0):
                            planet[5] += math.sqrt(settings[3]**3)*(DX*math.sqrt(Mf))/(DX*DX*math.sqrt(M))
                        if(DY!=0):
                            planet[6] += math.sqrt(settings[3]**3)*(DY*math.sqrt(Mf))/(DY*DY*math.sqrt(M))
                        if(DZ!=0):
                            planet[7] += math.sqrt(settings[3]**3)*(DZ*math.sqrt(Mf))/(DZ*DZ*math.sqrt(M))
                    '''
                    if(settings[18]):
                        if(DX!=0) and (DY!=0):
        ...
        if ((2*planet[8] + planet[5]) >= 0.5) or ((2*planet[8] + planet[5]) <= -0.5):
            planet[1] += 2*planet[8] + int(round(planet[5]))
            planet[8] = (planet[8] + int(round(planet[5])))
            planet[5] = 0
        if ((2*planet[9] + planet[6]) >= 0.5) or ((2*planet[9] + planet[6]) <= -0.5): 
            planet[2] += 2*planet[9] + int(round(planet[6]))
            planet[9] = (planet[9] + int(round(planet[6])))
            planet[6] = 0
        if ((2*planet[10] + planet[7]) >= 0.5) or ((2*planet[10] + planet[7]) <= -0.5): 
            planet[3] += 2*planet[10] + int(round(planet[7]))
            planet[10] = (planet[10] + int(round(planet[7])))
            planet[7] = 0
        planet[8] = planet[8] - int((settings[16]/100)*planet[8])
        planet[9] = planet[9] - int((settings[16]/100)*planet[9])
        planet[10] = planet[10] - int((settings[16]/100)*planet[10])
    ...
    for planet in planets:
        if(settings[11]):
            if (planet[1] <=2) or (planet[1]>=screenw-2):
                planet[8] = - planet[8]
                if(planet[1] < 0):
                    planet[1]+= 10
                    planet[8]+=1
                if(planet[1] > screenw):
                    planet[1]-=10
                    planet[8]-=1
            if (planet[2] <= 2) or (planet[2]>=screenh-2):
                planet[9] = -planet[9]
                if(planet[2] < 0):
                    planet[2]+= 10
                    planet[9]+=1
                if(planet[2] > screenh):
                    planet[2]-=10
                    planet[9]-=1
            if (planet[3] <= frontborder) or (planet[3]>=screend-2):
                planet[10] = -planet[10]
                if(planet[3] < frontborder):
                    planet[3]+= 10
                    planet[10]+=1
                if(planet[3] > screenh-2):
                    planet[3]-=10
                    planet[10]-=1
...
def eventhandle(event):
    ...
    if event.type == MOUSEBUTTONDOWN:
        ...
        if(textRectObj.collidepoint(event.pos)):
            ...
            for rect in controlRects:
                ...
                if(rect.collidepoint(event.pos)):
                    ...
                    if namelist[controlnum] == 'Gravity':
                        settings[17] = ((mousepos[0]-rect.left)/float(rect.width))*5
            ...
            extrainfo = [(random.randint(50,255),random.randint(50,255),random.randint(50,255)),
                mousepos[0],mousepos[1],random.randint(frontborder,screend-border),
                random.randint(settings[2],settings[3]),
                0,0,0,
                random.randint(-settings[4],settings[4]),random.randint(-settings[4],settings[4]),random.randint(-settings[4],settings[4])]

            planetarray.append(extrainfo)
            print(len(planetarray))
...
def controlsdraw():
    ...
    for rect in controlRects:
        ...
        if namelist[linenum] == 'FPS':
            ...
            pygame.draw.line(DISPLAYSURF, (0,0,0), ((rect.left + (settings[17]*rect.width/5)), rect.top),
                ((rect.left + (settings[17]*rect.width/5)), rect.bottom), 3)
